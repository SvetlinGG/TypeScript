    ...
    public listAllTowers(): string[]{
        ...
        for (const tower of this.towers) {
            ...
            if (tower instanceof CannonTower) {
                specificDetail = `Power: ${tower.power}`;
            } else if (tower instanceof MagicTower) {
                specificDetail = `SpellPower: ${tower.spellPower}`;
            } else if (tower instanceof IceTower) {
                specificDetail = `Freeze: ${tower.freeze}`;
            }

            result.push(
                `[${tower.type}] ${tower.name} (Level: ${tower.level}, ${specificDetail}) - Range: ${tower.calculateRange()}`
            );
        }
