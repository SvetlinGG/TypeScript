...
class CountedSet<T> implements CountableSet<T>{
    ...
    private currentCount = new Map<T, number>();

    add(item: T): void{
        this.currentCount.set(item, (this.currentCount.get(item) || 0) + 1)
    }

    remove(item: T): void{
        if (this.currentCount.has(item) && this.currentCount.get(item)! > 0){
            this.currentCount.set(item, this.currentCount.get(item)! -1)
        }
    }

    getNumberOfCopies(item: T): number{
        return this.currentCount.get(item) || 0;
    }
